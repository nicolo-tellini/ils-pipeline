...
vcfname=$(echo $j | rev | cut -d"/" -f1 | rev)

samp=$(echo $j | rev | cut -d"/" -f1 | rev | cut -d"." -f1)

control=$(grep -w $samp $BaseDir/todo.txt)
...
mv $file.eigenstratgeno $file.geno
